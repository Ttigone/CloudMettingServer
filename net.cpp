...
int Select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *execpfds,
  ...
  switch (sa->sa_family) {
    case AF_INET: {
      ...
      if (ntohs(sin->sin_port) > 0) {
        snprintf(str + strlen(str), size - strlen(str), ":%d",
                 ntohs(sin->sin_port));
      }
      return str;
    }
...
int Tcp_listen(const char *host, const char *service, socklen_t *addrlen) {
  ...
  hints.ai_flags =
      AI_PASSIVE;  // 设置了AI_PASSIVE标志，但没有指定主机名，那么return
                   // ipv6和ipv4通配地址
  hints.ai_family =
      AF_UNSPEC;  // 返回的是适用于指定主机名和服务名且适合任意协议族的地址
  hints.ai_socktype = SOCK_STREAM;
