...
void *thread_main(void *arg) {
  ...
  for (;;) {
    ...
    connfd = Accept(listenfd, cliaddr, &clilen);  // 接收客户端链接
    // unlock accept
    ...
    dowithuser(connfd);  // 处理用户请求
  }
  return NULL;
}

/// @brief 线程池处理请求, 找到空闲房间(子进程)
/// @param connfd
void dowithuser(int connfd) {
  char head[15] = {0};
  // read head
      ...
      if (msgtype == CREATE_MEETING) {
        ...
        if (datasize == 0 && tail == '#') {
          ...
          printf("create meeting  ip: %d.%d.%d.%d\n", (u_char)c[3],
                 (u_char)c[2], (u_char)c[1], (u_char)c[0]);
          if (room->navail <= 0) {
            // 没有空闲房间
            MSG msg;
        ...
        if (r < msgsize + 1) {
          ...
          if (head[msgsize] == '#') {
            ...
            int i;
            for (i = 0; i < nprocesses; i++) {
              if (room->pptr[i].child_pid == roomno &&
